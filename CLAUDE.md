...
- **Spellcheck yes, autocorrect no** — `isContinuousSpellCheckingEnabled` is a Settings toggle (off by default) and applies no automatic edits. Autocorrect (`isAutomaticSpellingCorrectionEnabled`) is deliberately hard-`false`: macOS NSTextView has **no per-view override** for sentence-start capitalization or period substitution — they follow the global `NSAutomaticCapitalizationEnabled` / `NSAutomaticPeriodSubstitutionEnabled` prefs, so turning on autocorrect drags both along and mangles plain text (`teh`→`Teh`). Confirmed via runtime probe: NSTextView does *not* respond to `setAutomaticCapitalizationEnabled:` and is not KVC-compliant for it. The six overridable `isAutomatic*` props (quotes, dashes, link, data, text-replacement, spelling-correction) exclude capitalization and period. Don't re-litigate — autocorrect-without-capitalization isn't reachable through public API.
